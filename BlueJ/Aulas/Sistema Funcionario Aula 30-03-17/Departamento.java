...
public class Departamento
{
    ...
    public void setChefe(Funcionario chefe){
        if(this.chefe.getDepartamentoChefiado() != null){
            System.out.println("Funcionario já é chefe");
        } else{
            this.chefe = chefe;
        }
    }
   ...
   public void addFuncionario(Funcionario funcionario){
    ...
    public float calcularFolhaDepartamento(){
        float valorTotalDepartamento = 0;
        for(int i = 0; i < funcionarios.size(); i++){
            valorTotalDepartamento += this.funcionarios.get(i).calcularSalario();
        }
        return valorTotalDepartamento;
    }
